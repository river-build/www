    ...
    <section className="hero-glow relative flex w-full flex-col items-start justify-center overflow-x-clip bg-gray-90 py-24 pb-0 pt-16 md:min-h-screen lg:items-center">
      <div className="relative mx-auto flex w-full max-w-7xl flex-col justify-between gap-8 px-4 py-8 md:px-8 lg:flex-row lg:py-[58px] xl:gap-14 xl:py-[200px]">
        <div className="flex w-full flex-col items-center justify-center md:px-0 lg:w-[62%] lg:items-start xl:w-[60%]">
          <a
            href={cmsData?.heroSection.topButtonUrl ?? '"https://blog.river.build"'}
            target="_blank"
          ...
          >
            ...
            <span className="relative z-10 inline-flex h-full w-full cursor-pointer items-center justify-center gap-2 rounded-[2000px] bg-gray-90 px-3 py-1 text-sm font-medium group-hover:bg-gray-70">
              <span className="text-gray-10">
                {cmsData?.heroSection.topButtonText ?? 'Introducing River Protocol'}
              </span>
          ...
          >
            {cmsData?.heroSection.heroSectionMainHeading ?? 'Build secure, real time'} <br />{' '}
            <span className="hero-text-gradient">
              {cmsData?.heroSection.heroSectionRainbowHeading ?? 'messaging apps.'}
            </span>
          </h1>
          ...
          >
            {cmsData?.heroSection.heroSectionDescription ??
          ...
          <div className="mt-8 flex items-center gap-3">
            <Button
              ...
              onClick={() =>
                window.open(
                  cmsData?.heroSection.leftButtonUrl ?? 'https://docs.river.build/introduction',
                  '_blank',
                )
              }
            >
              <div className="flex items-center gap-1">
                <span>{cmsData?.heroSection.leftButtonText ?? 'Read the Docs'}</span>
            ...
            <Button
              ...
              onClick={() =>
                window.open(
                  cmsData?.heroSection.rightButtonUrl ??
                    'https://docs.river.build/run/introduction',
                  '_blank',
                )
              }
            >
              <div className="flex items-center gap-2">
                <CryptoCurrency />
                <span>{cmsData?.heroSection.rightButtonText ?? 'Run a Node'}</span>
              </div>
            </Button>
          </div>
        </div>
